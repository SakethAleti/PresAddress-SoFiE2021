...
class LaTeXRenderer(Renderer):
    ...
    def generate_header(self, only_tabular=False):
        ...
        if self.dep_var_name is not None:
            ...
            header += '\\cr \\cline{' + str(2) + '-' + str(self.num_models + 1) + '}\n'
    ...
    def generate_cov_rows(self, cov_name):
        ...
        if self.show_precision:
            ...
            if self.show_tstats:
                cov_text += self.generate_cov_tstats(cov_name)
        else:
            cov_text += '& '
    ...
    def generate_cov_tstats(self, cov_name):
        cov_text = '\\\\\n'

        for md in self.model_data:
            if cov_name in md['cov_names']:
                cov_text += '& $['
                cov_text += self._float_format(md['t_stats'][cov_name])
                cov_text += ']$ '
            else:
                cov_text += '& '
